...
G_spat = re.compile(
    ...
    "(-|_)(fhd|hd|sd|1080p|720p|4K|x264|x265|uncensored|hack|leak|流出|无码流出|字幕)",
    re.IGNORECASE)
...
def get_number(debug: bool, file_path: str) -> str:
    ...
    try:
        ...
        if file_number:
            ...
            return file_number
        elif (filepath.count('-') == 1 and '-C' not in filepath.upper()) or filepath.count('-') >= 2 or '_' in filepath:  # 普通提取番号 主要处理包含减号-和_的番号
            filepath = G_spat.sub("", filepath)
